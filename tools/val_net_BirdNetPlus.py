...
2. Evaluate inside of the algorith using an algorithm like https://github.com/traveller59/kitti-object-eval-python, adapting the input
3. Visualize and save the images resulting in both BEV and 3D
'''
...
def parse_args():
    ...
    parser.add_argument(
        '--python_eval', help="Perform KITTI evaluation inside of the script", default=False, action="store_true")
    parser.add_argument(
...
def main(config_file, ann_val, write, python_eval, img2show, save_img, eval_chkp, force_test, score_thresh , nms_thresh, kitti_root ):
    # KITTI paths
    kitti_im_path = kitti_root+'/image_2'
    kitti_lbl_path = kitti_root+'/label_2'
    kitti_calib_path = kitti_root+'/calib'
    # If the flag is not true, dont load the libraries either
    if python_eval:
        # To perform KITTI evaluation inside: https://github.com/traveller59/kitti-object-eval-python
        sys.path.insert(0, os.path.join(home,'kitti-object-eval-python'))
        from kitti_common import get_label_annos_mod, get_label_annos
    ...
    if python_eval:
        ...
        gt_annos = get_label_annos(kitti_lbl_path, val_image_ids)
    ...
    if force_test:
        ...
        length = len(toeval)//2
        toeval = toeval[length-1::eval_chkp]
        f_eval = [folder.split('_')[1].split('.')[0] for folder in toeval]
    ...
    for checkpoint, eval_folder in zip(toeval,f_eval):
        ...
        for image_id, d in enumerate(val_bv_dicts):
            ...
            if not os.path.exists(file) or c in sample_idx:
                # Inference
            ...
            if c in sample_idx:
                # Evaluation for 2D and 3D as well
                # Change BV aspect
                ...
                for i, obj in enumerate(obj_anns):
                    kitti_im, im, _ = _draw_projection_obstacle_to_cam(obj, calib_file, bvres, only_front, True, kitti_im, im)
            ...
            elif c > max(sample_idx) and not python_eval:
                break

        if python_eval:
            dt_annos = get_label_annos_mod(kitti_results)
            print(get_official_eval_result(gt_annos, dt_annos, [0,3,5]))

if __name__ == '__main__':
    args = parse_args()

    main(args.config_file, args.ann_val, args.write, args.python_eval, args.img2show, args.save_img, args.eval_chkp, args.force_test, args.score, args.nms, args.kitti_root)