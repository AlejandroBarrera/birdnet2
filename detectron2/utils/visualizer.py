...
class Visualizer:
    ...
    def draw_instance_predictions(self, predictions):
        ...
        viewpoints = predictions.viewpoint if predictions.has("viewpoint") else None
        ...
        if self._instance_mode == ColorMode.SEGMENTATION and self.metadata.get("thing_colors"):
            ...
            labels_cls = [lbl.split(' ')[0] for lbl in labels]
            colors = [color_dict[lbl] for lbl in labels_cls]
            alpha = 0.8
    ...
    def draw_dataset_dict(self, dic):
        ...
        if annos:
            ...
            colors = [color_dict[lbl] for lbl in labels]
            self.overlay_instances(labels=None, boxes=boxes, masks=masks, keypoints=keypts, assigned_colors=colors, alpha=0.4)
        ...
        *,
        ...
        if boxes is not None and boxes.shape[1] == 5:
            return self.overlay_rotated_instances(
                boxes=boxes, labels=labels, viewpoints=viewpoints, assigned_colors=assigned_colors
            )
        ...
        for i in range(num_instances):
            ...
            if viewpoints is not None:
                x0, y0, x1, y1 = boxes[i]
                vp_base = ((x1-x0)/2.+x0, (y1-y0)/2.+y0)
                self.draw_vp(viewpoints[i], vp_base, color=color)
            if masks is not None:
    ...
    def overlay_rotated_instances(self, boxes=None, labels=None, viewpoints=None, assigned_colors=None):
        """
        Args:
        ...
        # Re-order overlapped instances in descending order.
        if viewpoints is not None:
            viewpoints = viewpoints[sorted_idxs]
        ...
        for i in range(num_instances):
            if viewpoints is not None:
                ...
                vp_base = ((x1-x0)/2.+x0, (y1-y0)/2.+y0)
                self.draw_vp(viewpoints[i], vp_base, color=colors[i])
    ...
    def draw_vp(self, viewpoint, vp_base, color):
        arrow_len = 30
        bin_dist = np.linspace(-math.pi,math.pi,17) #16 bins for each class (bins*n_classes)
        bin_res = (bin_dist[1]-bin_dist[0])/2.
        bins = [bin_dist[i] for i in range(len(bin_dist)-1)]
        rad = bins[viewpoint]
        x1 = arrow_len*math.cos(rad)
        y1 = arrow_len*math.sin(rad)
    ...
    def draw_rotated_box_with_label(
        ...
        x0, y0, x1, y1, angle = rotated_box
        cnt_x,cnt_y = ((x1-x0)/2.+x0, (y1-y0)/2.+y0)
        w = x1-x0
        h = y1-y0
        area = w * h
        ...
        if label is not None:
            ...
            self.draw_text(label, text_pos, color=label_color, font_size=font_size, rotation=0)
